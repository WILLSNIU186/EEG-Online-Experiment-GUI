...
stimulus_sequence = [1, 4, 3, 2, 5, 6, 7]
...
def get_frame_movement_phase(frame_number, stimulus_frequency):
    movement_phase = ((np.pi/2)+(np.pi/2)*np.sin((2*np.pi*frame_number*(stimulus_frequency/(2*screen_refresh_rate)))-(np.pi/2)))/np.pi*0.5
...
class Presenter:
    ...
    def run_ssvep_protocol(self, stim_type='ssvep', stimulus_sequence=stimulus_sequence, 
        ...
        for sequence_idx, stimulus_id in enumerate(stimulus_sequence):
            ...
            message.pos = (
            positions_list[stimulus_id - 1][0] / screen_size[0] * 2, positions_list[stimulus_id - 1][1] / screen_size[1] * 2)
            win.flip()
                ...
                if not event.getKeys():
                    for stimulus_idx, stimulus in enumerate(stimulus_list):
                        if stim_type=='ssvep':
                            ...
                            stimulus.radialPhase = get_frame_movement_phase(frame_number, frequencies_list[stimulus_idx])
                            stimulus.draw()
