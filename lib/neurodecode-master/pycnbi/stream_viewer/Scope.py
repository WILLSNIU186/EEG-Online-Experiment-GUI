...
from pycnbi.stream_viewer.ui_mainwindow_Viewer_4 import Ui_MainWindow
from pycnbi.stream_viewer.ui_subjectview import Ui_SV
from pycnbi.stream_recorder.stream_recorder import record, run, batch_run, run_gui
from tushar import RecordingFromHardWare
from random import randrange
from GetMRCPTemplate import GetMRCPTemplate
import pdb
...
path2_viewerFolder = 'D:\neurodecode-master\pycnbi\stream_viewer'
...
class Scope(QMainWindow):
    def __init__(self, amp_name, amp_serial, state=mp.Value('i', 1), queue=None):
        ...
        self.state = state
        self.init_scope()
        self.recordingFromHardWare = RecordingFromHardWare()
    ...
    def init_panel_GUI(self):
        ...
        self.ui.pushButton_stop_SV.clicked.connect(self.onClicked_button_stop_SV)
        
        self.ui.pushButton_temp_add.clicked.connect(self.onClicked_button_temp_add)
    ...
    def onClicked_button_start_SV(self):
        global s
        self.SV_time = 0
        self.start_exp_bt_clicked = True
        self.window.show()
    ...
    def init_SV_GUI(self):
        ...
        self.starttime = 0
        self.SV_time = 0
        self.start_exp_bt_clicked = False

        
    def Update_SV_image(self):
    
        print("SV time {}".format(self.SV_time))
        if self.SV_time % 10 == 0:
            #Idle
            self.SV_window.LBimage.setPixmap(QtGui.QPixmap("icon/idle.png"))
        elif self.SV_time % 10 == 4:
            #Focus
            self.SV_window.LBimage.setPixmap(QtGui.QPixmap("icon/focus.png"))
        elif self.SV_time % 10 == 5:
            #Prepare
            self.SV_window.LBimage.setPixmap(QtGui.QPixmap("icon/prepare.png"))
        elif self.SV_time % 10 == 6:
            #Two
            self.SV_window.LBimage.setPixmap(QtGui.QPixmap("icon/two.png"))
        elif  self.SV_time % 10 == 7:
            #One
            self.SV_window.LBimage.setPixmap(QtGui.QPixmap("icon/one.png"))
        elif self.SV_time % 10 == 8:   
            #Task
            self.SV_window.LBimage.setPixmap(QtGui.QPixmap("icon/task.png"))
        self.SV_time += 1
        
    def update_MRCP_plot(self):
        self.recordingFromHardWare.set_MRCP_window_size(6)
        if self.SV_time % 10 == 2 and self.SV_time != 2:
            self.raw_trial_MRCP = self.recordingFromHardWare.current_window  
            print("RAW MRCP: {}".format(self.raw_trial_MRCP))
            print("RAW MRCP shape: {}".format(self.raw_trial_MRCP.shape))
            getMRCPTemplate = GetMRCPTemplate(self.raw_trial_MRCP,self.sr.sample_rate)
            self.processed_trial_MRCP = getMRCPTemplate.preprocess()  
            print("processed_MRCP: {}".format(self.processed_trial_MRCP))
            #save each MRCP into total MRCP list
        ...
        if self.SV_time % 10 == 5 and self.SV_time != 5:
            self.line_width = 2
            self.MRCP_plot(self.processed_trial_MRCP)
            #concatenate MRCP
#            self.total_trials_MRCP.append(self.raw_trial_MRCP)
    ...
    def MRCP_plot(self,about_to_plot_MRCP):
       # pdb.set_trace()
        ...
        self.ui.graphicsView.plot(x,y,pen = pg.mkPen(color = (R,G,B) , width = self.line_width))
    ...
    def onClicked_button_rec(self):
        ...
        if  self.recordingFromHardWare.isRecordingIsRunning()==False:
            ...
            self.recordingFromHardWare.startRecording()
    # =============================================================================
    #         batch_run(record_dir, amp_name, amp_serial)
    # =============================================================================
            
            
            if (len(self.ui.lineEdit_recFilename.text()) > 0):
                if ".gdf" in self.ui.lineEdit_recFilename.text():
                    self.ui.pushButton_stoprec.setEnabled(True)
    ...
    def onClicked_button_stoprec(self):
        ...
        self.timer.stop()
        self.recordingFromHardWare.stopRecording()
...
# =============================================================================
    ...
    def onClicked_button_temp_add(self):
        self.get_input_temp()
        pdb.set_trace()
        #check if these items already in the list
        repeat_item_checker = []
        repeat_item_checker = [x for x in self.display_temp_list if x in self.input_temp_list]
        if len(repeat_item_checker) == 0:
            #add items from input list
            self.display_temp_list = self.display_temp_list + self.input_temp_list
            self.plot_display_temp()
        else:
            self.ui.statusBar.showMessage("this temp is alreay displayed")
    ...
    def label_available_temp(self):
        pass
    
    def label_disp_temp(self):
        pass
    
    def input_channel_selector(self):
        pass
        
    def onClicked_button_stop_SV(self):
        self.ui.label_content_available_temp.setText("{} - {}".format(self.temp_counter_list[0],self.temp_counter_list[-1]))
        self.ui.label_content_Disp_temp.setText("{} - {}".format(self.temp_counter_list[0],self.temp_counter_list[-1]))
        self.ui.label_content_current_temp.setText(" ")
        self.total_MRCP_inds = list(range(len(ex.total_trials_MRCP)))
